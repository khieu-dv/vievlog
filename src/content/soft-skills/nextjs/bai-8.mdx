...
```mermaid
flowchart TD
    A[Tạo Component] --> B{Cần tương tác?}
    B -->|Có| C{Dùng hooks?}
    B -->|Không| D[Server Component]
    C -->|Có| E[use client]
    C -->|Không| F{Browser API?}
    F -->|Có| E
    F -->|Không| D
```
...
sequenceDiagram
    ...
    CDN->>Server: Revalidate in background
    Server->>DB: Fetch new data
    DB->>Server: Return data
    Server->>CDN: Update cache
    
    Note over CDN: Fresh content ready
```
...
```mermaid
graph LR
    A[Server HTML] --> B[Download JS]
    B --> C[React Hydration]
    C --> D[Interactive]
    
    subgraph "Hydration Steps"
        E[Static HTML]
        F[Event Listeners]
        G[State Init]
        H[Ready]
