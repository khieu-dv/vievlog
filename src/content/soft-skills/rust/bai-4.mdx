...
```mermaid
flowchart TD
    A[Bắt đầu] --> B{Điều kiện if}
    B -->|true| C[Thực thi khối if]
    B -->|false| D{Có else if?}
    D -->|true| E{Điều kiện else if}
    D -->|false| F[Thực thi khối else]
    E -->|true| G[Thực thi khối else if]
    E -->|false| F
    C --> H[Tiếp tục]
    G --> H
    F --> H
...
graph TD
    ...
    C -->|continue 'outer| E[Tiếp tục outer loop]
    C -->|break| F[Thoát inner loop]
    C -->|continue| G[Tiếp tục inner loop]
```
