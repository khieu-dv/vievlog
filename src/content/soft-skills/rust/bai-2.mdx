...
graph TD
    ...
    A --> C[Block Comments /* */]
    A --> D[Doc Comments ///]
    B --> E[Dành cho ghi chú ngắn]
    C --> F[Dành cho ghi chú nhiều dòng]
    D --> G[Tạo documentation]
```
...
```mermaid
graph TD
    A[Khai báo biến lần 1] --> B[let x = 5]
    B --> C[Khai báo biến lần 2]
    C --> D[let x = x + 1]
    D --> E[Biến mới che khuất biến cũ]
    E --> F[x = 6]
```
...
graph TD
    ...
    D --> E[Không truy cập được từ outer]
    B --> F[Variables trong outer scope]
    F --> G[Truy cập được từ inner]
```
