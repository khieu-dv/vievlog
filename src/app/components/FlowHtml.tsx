...
export default function App({ category = 0 }: FlowDiagramProps) {
  ...
  const [htmlLoadError, setHtmlLoadError] = useState<string | null>(null);
  ...
  const onNodeClick = useCallback((event: React.MouseEvent, node: Node) => {
    ...
    const url = node.data?.url_html ? String(node.data.url_html) : "";

    if (!url) {
      setError("No HTML URL available for this node");
      return;
    }
  ...
  const handleDragStart = useCallback((e: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) => {
    ...
    if ((e.target as HTMLElement).closest(".popup-header") && !(e.target as HTMLElement).closest(".popup-controls")) {
  ...
  const handleIframeError = () => {
    setHtmlLoadError("Failed to load HTML. Please try again.");
  };

  const reloadHtml = () => {
    if (iframeRef.current) {
      iframeRef.current.src = popupInfo.url;
      setHtmlLoadError(null);
    }
  };
  ...
  return (
    <div className="w-full h-[600px]">
      <ReactFlow<CustomNodeType, CustomEdgeType>
        ...
        elementsSelectable={true} // Không cho phép chọn các phần tử
      ...
      {showPopup && (
        ...
        >
          <div className="popup-header flex justify-between items-center p-2 bg-gray-100 dark:bg-gray-700 rounded-t-lg cursor-move">
            ...
            <h3 className="text-lg font-medium flex-1 text-center">Kéo để di chuyển</h3>
          ...
          {isLoading && (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">Loading HTML...</div>
            </div>
          )}
